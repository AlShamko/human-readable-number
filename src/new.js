function toReadable(number) {
	...
	if(number < 100) {
		...
		const b = myArrayF[odinki[1]] === 'zero' ? "" : myArrayF[odinki[1]];
		const result = a + ' ' + b;
	...
	if(number < 120) {
		...
		const result = (z + ' ' + myArrayOne[number - 100]);
		return result
	}
	...
	if(number < 200) {
		...
		const b = myArrayF[odinki[2]] === 'zero' ? "" : myArrayF[odinki[2]];
		const result = (z + ' ' + a + ' ' + b);
		return result.replace(/ +/g, ' ').trim()
	}  
}

console.log(toReadable(119));